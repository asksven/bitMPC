...
public abstract class GestureListView extends ListView
{
	...
	private static class MovingThread extends
			AsyncTask<GestureListView, Integer, Integer>
	{
	...
	private static final float DELETE_GEST_LONG = 80; // delete gesture long in
														// pixel
	private static final float DRAG_GEST_LONG = 80; // drag gesture long in
													// pixel
	private static final float EDGE_GESTURE_ERROR = 5; // percent error in edge
														// detection
	...
	{
		int aux;
		if ((aux = pointToPosition((int) _ev.getX(), (int) _ev.getY())) != dragPosition
				&& aux != INVALID_POSITION)
		{
	...
	private void checkScroll(boolean _iteration)
	{
		...
		if ((status == Status.MOVING)
				&& (_iteration || movement.getStatus() != AsyncTask.Status.RUNNING))
		{
			...
			} else if (dragPosition == getLastVisiblePosition()
					&& dragPosition < getAdapter().getCount() - 2)
			{
				swap(dragPosition, dragPosition + 1);
				dragPosition = dragPosition + 1;
				// FIXME que pasa con los paddings?
				setSelectionFromTop(dragPosition, getHeight()
						- dragPositionHeight);
				movement = new MovingThread();
				movement.execute(this);
			}
	...
	private boolean touchUp(MotionEvent _ev)
	{
		if ((dragPosition != INVALID_POSITION)
				&& (_ev.getX() - x > DELETE_GEST_LONG)
				&& (_ev.getX() > getWidth() * (100 - EDGE_GESTURE_ERROR) / 100)
				&& (dragPosition == pointToPosition((int) _ev.getX(),
						(int) _ev.getY())))
			for (RemoveListener listener : removeListeners)
				listener.remove(dragPosition);
		if ((dragPosition != INVALID_POSITION)
				&& (status == Status.NORMAL)
				&& (x - _ev.getX() > DRAG_GEST_LONG)
				&& (_ev.getX() < getWidth() * (EDGE_GESTURE_ERROR / 100))
				&& (dragPosition == pointToPosition((int) _ev.getX(),
						(int) _ev.getY())))
			drag();
		if (status == Status.MOVING)
	...
	private void move()
	{
		...
		dragPositionHeight = getChildAt(
				dragPosition - getFirstVisiblePosition()).getHeight();
	}

	private boolean drop()
	{
		for (DropListener listener : dropListeners)
			listener.drop(dragPosition);
		status = Status.NORMAL;
		return true;
	}
	...
	private void drag()
	{
		for (DragListener listener : dragListeners)
			listener.drag(dragPosition);
		status = Status.DRAG;
	}

	public void addRemoveListener(RemoveListener _listener)
	{
		removeListeners.add(_listener);
	}

	public void removeRemoveListener(RemoveListener _listener)
	{
		removeListeners.remove(_listener);
	}

	public void addDragListener(DragListener _listener)
	{
		dragListeners.add(_listener);
	}

	public void removeDragListener(DragListener _listener)
	{
		dragListeners.remove(_listener);
	}

	public void addDropListener(DropListener _listener)
	{
		dropListeners.add(_listener);
	}

	public void removeDropListener(DropListener _listener)
	{
		dropListeners.remove(_listener);
	}
