...
public class RSSParser extends DefaultHandler
{
	...
	private boolean items, rss, intitle, initem, indescription, initemtitle,
			initemdescription, inimage, inurlimage;

	private StringBuffer image, title, program_title, description,
			program_description;
			...
			Attributes _attributes) throws SAXException
	{
		...
		if ("rss".equals(_localName))
			rss = true;
		if ("title".equals(_localName))
		{
			if (initem)
				initemtitle = true;
			else if ("".equals(title.toString()))
				intitle = true;
		}
		if ("description".equals(_localName))
		{
			...
			else if ("".equals(description.toString()))
				indescription = true;
		}
		...
		if ("image".equals(_localName) && !initem
				&& "".equals(image.toString()))
		{
			if (_attributes.getIndex("href") != -1)
				image.append(_attributes.getValue("href"));
			else
				inimage = true;
		}
		if ("url".equals(_localName) && inimage)
			inurlimage = true;
	}
	...
	{
		if (intitle)
			title.append(_ch, _start, _length);
		if (indescription)
			description.append(_ch, _start, _length);
		if (items && initemtitle)
			program_title.append(_ch, _start, _length);
		if (items && initemdescription)
			program_description.append(_ch, _start, _length);
		if (inurlimage)
			image.append(_ch, _start, _length);
	}
			...
			throws SAXException
	{
		...
		if ("item".equals(_localName))
		{
			...
			if (items && rss && !"".equals(program_title)
					&& !"".equals(program_description) && program_url != null)
			{
				...
				handler.sendMessage(Message.obtain(handler,
						BitMPCHandler.MESSAGE_RSS_PROGRAM, program));
			}
			initem = false;
		}
		if ("description".equals(_localName))
	...
	public void endDocument() throws SAXException
	{
		if (rss && !"".equals(title) && !"".equals(description))
		{
			...
			if (!"".equals(image.toString()))
				item.addImage(image.toString());
			handler.sendMessage(Message.obtain(handler,
					items ? BitMPCHandler.MESSAGE_UPDATE_RSS
							: BitMPCHandler.MESSAGE_NEW_RSS, item));
		} else
		{
			handler.sendMessage(Message.obtain(handler,
