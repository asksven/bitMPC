...
public class BrowseItemAdapter extends BitMPCAdapter<BrowseItem>
{
	...
	public BrowseItemAdapter(BitMPC _context)
	{
		super(_context);
	}
	...
	{
		if (_text.startsWith("file") || _text.startsWith("directory")
				|| _text.startsWith("playlist"))
			addItem(new BrowseItem(_text));
		else
			getLast().add(_text);
	}
	...
	protected View getView(int _position, BrowseItem _item, View _view)
	{
		if (_view == null)
			_view = getContext().getLayoutInflater().inflate(
					R.layout.icon_listitem, null);
		ImageView image = (ImageView) _view
				.findViewById(R.id.icon_listitem_image);
		switch (_item.getType())
		{
		case DIRECTORY:
			image.setImageDrawable(getContext().getResources().getDrawable(
					R.drawable.browse_folder));
			break;
		case FILE:
			image.setImageDrawable(getContext().getResources().getDrawable(
					R.drawable.browse_file));
			break;
		case PLAYLIST:
			image.setImageDrawable(getContext().getResources().getDrawable(
					R.drawable.browse_playlist));
			break;
		default:
			break;
		}
		((TextView) _view.findViewById(R.id.icon_listitem_text)).setText(_item
				.toString());
		return _view;
	}
