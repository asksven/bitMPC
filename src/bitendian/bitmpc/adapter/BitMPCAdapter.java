 ...
 *
 * @param <T>
 *            Generic type to be managed by adapter.
 */
public abstract class BitMPCAdapter<T> extends BaseAdapter implements
		ListAdapter
{
	 ...
	 * 
	 * @param _position
	 *            Position of the item whose the data we want winthin the
	 *            Adapter's data set.
	 * @return The data at the specified position casted to generic T type.
	 */
	public T getTItem(int _position)
	{
		return list.get(_position);
	}
	...
	public void setItem(int _position, T _item)
	{
		list.set(_position, _item);
	}
	 ...
	 * 
	 * @param _position
	 *            Where to delete item from data set.
	 */
	 ...
	 * 
	 * @param _os
	 *            Where to write persistent attributes.
	 */
	...
	public void restore()
	{
		try
		{
			ObjectInputStream is = new ObjectInputStream(
					context.openFileInput(getFilename()));
	 ...
	 * 
	 * @param _size
	 *            Max number of head elements that will remain in Adapter
	 */
	public void head(int _size)
	{
		for (int i = list.size() - 1, size = _size; i >= size; i--)
			list.remove(i);
	}
