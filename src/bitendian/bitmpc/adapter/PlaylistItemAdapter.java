...
public class PlaylistItemAdapter extends BitMPCAdapter<PlaylistItem>
{
	...
	{
		if (_view == null)
			_view = getContext().getLayoutInflater().inflate(R.layout.listitem,
					null);
		...
		if (_item.isDeleting() || _item.isMoving())
		{
			...
			main.setText(getContext().getString(
					_item.isDeleting() ? R.string.playlist_deleting
							: R.string.playlist_moving));
		...
		} else
		{
			...
			sub.setText(artistname == null ? getContext().getString(
					R.string.unknown_artist) : artistname);
		}
